...
import { fetchRawCleaningLogs } from '../../api/rawCleaningLogs';
import { Badge } from '../ui';
import type { BadgeVariant } from '../ui/Badge';
import { useTheme } from '../../theme';
import { radius, spacing } from '../../theme/spacing';
import { typography } from '../../theme/typography';
import { RawCleaningLog } from '../../types/rawCleaningLogs';
import {
  formatApiDateLabel,
  formatDateTimeIST,
...
function getTopicVariant(topic: string): BadgeVariant {
  ...
  if (value.includes('cycle')) return 'info';
...
function LogCard({ log }: { log: RawCleaningLog }) {
  ...
  return (
    ...
    >
      <View style={styles.logHeader}>
        <Badge label={log.topic} variant={getTopicVariant(log.topic)} size="sm" />
      ...
      <View style={styles.logMetrics}>
        ...
        <View style={styles.metricCell}>
          ...
          <Text style={[styles.metricValue, { color: colors.badge.warning.text }]}>
            {log.snr || '—'}
          </Text>
        </View>
        <View style={styles.metricCell}>
...
export function RobotRawCleaningLogs({ robotNo, active }: Props) {
  ...
  const [logs, setLogs] = useState<RawCleaningLog[]>([]);
  ...
  const loadLogs = useCallback(
    async (targetPage: number) => {
      ...
      try {
        const result = await fetchRawCleaningLogs({
          robotNo,
          page: targetPage,
          limit: PAGE_LIMIT,
          startDate,
        ...
        setTotal(0);
        setError(
          err instanceof Error ? err.message : 'Failed to load raw cleaning logs',
        );
      } finally {
        setLoading(false);
      }
  ...
  const iosDatePickerModal =
    dateField && Platform.OS === 'ios' ? (
      <Modal transparent animationType="slide" visible onRequestClose={() => setDateField(null)}>
  ...
  return (
    <View style={styles.wrapper}>
      ...
      {loading ? (
        ...
        <View style={styles.centerState}>
          ...
          <Text style={[styles.emptyText, { color: colors.textMuted }]}>
            No raw cleaning logs for {robotNo} in this date range.
          </Text>
      ...
      <View style={styles.pagination}>
        <Pressable
          onPress={() => hasPrevPage && setPage((current) => Math.max(1, current - 1))}
