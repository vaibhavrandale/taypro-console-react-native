...
import {
  RawCleaningLog,
  RawCleaningLogsParams,
  RawCleaningLogsResult,
} from '../types/rawCleaningLogs';
...
function emptyResult(page: number, limit: number): RawCleaningLogsResult {
  return {
    logs: [],
    total: 0,
    page,
...
function normalizeResponse(
  ...
  if (payload.success === false) {
    ...
    throw new Error(message || 'Failed to load raw cleaning logs');
  }

  const logs = asArray<RawCleaningLog>(payload.data);
...
export async function fetchRawCleaningLogs(
  params: RawCleaningLogsParams,
): Promise<RawCleaningLogsResult> {
  ...
  const response = await apiFetch('/rawcleaninglogs/get-raw-cleaning-logs', {
    method: 'POST',
    headers: {
